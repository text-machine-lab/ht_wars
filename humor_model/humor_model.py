...
@ex.config
def my_config():
    learning_rate = np.random.uniform(.00005, .000001)
    num_epochs = int(np.random.uniform(1.0, 5.0))
    dropout = np.random.uniform(.6, 1.0)


def build_embedding_humor_model(vocab_size):
    ...
    tf_tweet_pair_emb = tf.concat(1, [tf_first_tweet_encoder_output, tf_second_tweet_encoder_output, tweet1_conv_emb,
                                      tweet2_conv_emb])  # , tweet1_conv_emb, tweet2_conv_emb])

    tweet_pair_emb_size = int(tf_tweet_pair_emb.get_shape()[1])

    tf_tweet_dense_layer1, _, _ = create_dense_layer(tf_tweet_pair_emb, tweet_pair_emb_size,
                                                     tweet_pair_emb_size * 3 / 4, activation='relu', name='layer_1')
    tf_tweet_dense_layer1_dropout = tf.nn.dropout(tf_tweet_dense_layer1, keep_prob=tf_dropout_rate)
    tf_tweet_dense_layer2, _, _ = create_dense_layer(tf_tweet_dense_layer1_dropout, tweet_pair_emb_size * 3 / 4,
...
def train_on_all_other_hashtags(model_vars, trainer_vars, hashtag_names, hashtag_name, hashtag_datas, n_epochs=1,
    ...
    for epoch in range(n_epochs):
        ...
        print 'Saving..'
        saver.save(sess, os.path.join(EMBEDDING_HUMOR_MODEL_DIR, 'emb_humor_model'),
...
@ex.main
def main(learning_rate, num_epochs, dropout):
    ...
    g = tf.Graph()
    with g.as_default():
        model_vars = build_embedding_humor_model(vocab_size)
