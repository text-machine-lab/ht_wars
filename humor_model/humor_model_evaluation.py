...
def main():
    ...
    if len(sys.argv) < 4:
        print 'Usage: humor_model_evaluation [tweet_dir] [tweet_pair_dir] [output_dir]'
    else:
        tweet_dir = sys.argv[1]
        tweet_pair_dir = sys.argv[2]
        output_dir = sys.argv[3]
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        model_vars = humor_model.build_embedding_humor_model()

        sess = tf.InteractiveSession(config=tf.ConfigProto(gpu_options=GPU_OPTIONS))
        saver = tf.train.Saver(max_to_keep=10)
        # Restore model from previous save.
        ckpt = tf.train.get_checkpoint_state(EMBEDDING_HUMOR_MODEL_DIR)
        ...
        for hashtag_name in hashtag_names:
            accuracy, [np_predictions, first_tweet_ids, second_tweet_ids] = \
                predict_on_hashtag(sess, model_vars, hashtag_name, tweet_pair_dir)
