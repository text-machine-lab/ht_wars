...
from config import EMBEDDING_HUMOR_MODEL_DIR, HUMOR_MAX_WORDS_IN_TWEET, HUMOR_MAX_WORDS_IN_HASHTAG, GLOVE_EMB_SIZE, \
    TWEET_SIZE, PHONETIC_EMB_SIZE
...
def my_config():
    ...
    num_epochs = 1  # int(np.random.uniform(1.0, 5.0))
    dropout = .7  # np.random.uniform(.6, 1.0)


def build_embedding_humor_model(vocab_size, use_embedding_model=True, use_character_model=True):
...
def main(learning_rate, num_epochs, dropout):
    ...
    g = tf.Graph()
    with g.as_default():
        model_vars = build_embedding_humor_model(vocab_size, use_embedding_model=False)
