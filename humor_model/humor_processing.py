...
def convert_tweet_to_gloves(tweets, word_to_glove, max_number_of_words, glove_size):
    np_tweet_gloves = np.zeros([len(tweets), max_number_of_words * glove_size])
    for i in range(len(tweets)):
        tokens = tweets[i].split()
